...
create table if not exists holiday_region_cache_status (
  provider text not null default 'calendarific',
  country_code text not null,
...
create index if not exists public_holidays_lookup_idx
  on public_holidays (country_code, region_code, year, holiday_date);

insert into holiday_regions (provider, country_code, region_code, name, payload)
values
...
insert into holiday_region_cache_status (provider, country_code, status)
